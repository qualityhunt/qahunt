        ...
        <div class="col-12">
            <div class="card">
                ...
                <div class="card-content">
                    <div class="card-body">
<h4>Danışma ve İstişare Kurulu </h4>
                        <a href="{{   route('admin.advisory.create')   }}" class="btn btn-primary mb-2 float-right"><i
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td align="left"> <img style="height: 50px;width: 50px;" class="img-circle"
                                                src="{{  URL::to('uploads/advisory/')}}/{{ $ad->image }}"></td>


                                        <td>{{ $ad->name_tr }}</td>

                                        <td>{{ $ad->position_tr }}</td>

                                        <td>{!! $ad->text_tr !!}</td>

                                        <td>
                                            <a href="{{   route('admin.advisory.edit',$ad->id) }}"> <i
                                                    class="feather icon-edit font-medium-5"></i> </a>
                                            <a href=""
                                                onclick="if(confirm('Silmek İstediğinize Emin Misiniz?')){event.preventDefault();document.getElementById('delete-form-{{ $ad->id }}').submit();}else{event.preventDefault();}">
                                                <i class="feather icon-trash  font-medium-5"> </i></a>
                                            <form id="delete-form-{{ $ad->id }}" method="post"
                                                action="{{ route('admin.advisory.destroy',$ad->id) }}">
                                                {{ csrf_field() }}
                                                {{ method_field('DELETE') }}
                                            </form>
